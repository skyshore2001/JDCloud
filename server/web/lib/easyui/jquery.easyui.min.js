...
if (window.WUI) { // >>> LJ: lang 标题使用html时支持翻译
	WUI.enhanceWithin(_239);
}
if(opts.iconCls){
...
// >>> LJ: lang
if (opts.title && window.T) {
opts.title = T(opts.title); 
}
_230(this);
$(this).show();
if(opts.doSize==true){
...
// >>>LJ: 1. bugfix: rule名带数字无法使用；2. validator在值为空时也应调用(扩展rule.checkEmpty选项)，这样可实现根据其它字段判断是否为必填的需求。
var _55b=/([a-zA-Z_0-9]+)(.*)/.exec(_558);
var rule=opts.rules[_55b[1]];
if(rule&&(rule.checkEmpty||_55a)){
// <<<
...
// if(_730!=_729)  // bugfix: 优化后, 这里必须全部设置, 否则行宽可能不对!
{
_730=Math.max(_730,_729)+1;
arr.push({tr1:tr1, tr2:tr2, height:_730});
}
}
$.each(arr, function (i, e) {
	e.tr1.css("height", e.height);
	e.tr2.css("height", e.height);
});
};
function _72e(cc){
var _731=0;
...
// >>>LJ: perfix
function quickAutoSize(dgObj, dgData)
{
	var jtbl = $(dgObj);
	var dc = dgData.dc;
//	var opts = dgData.options;
	var hdCells =dc.view.find("div.datagrid-header td[field] div.datagrid-cell");
	var bdCells = dc.body2.find("tr:first td[field] div.datagrid-cell");
	if (hdCells.size() == 0 || bdCells.size() == 0)
		return;
	var arr = [];
	var cols = jtbl.datagrid("getColumnFields");
	$.each(cols, function (i, col) {
		var ja = $(hdCells[i]);
		var jb = $(bdCells[i]);
		var width = Math.max(ja.width(), jb.width());
		var colOpt = jtbl.datagrid("getColumnOption", col);
		colOpt.width = width;
		colOpt.boxWidth = width;
		colOpt.deltaWidth = 0;
		colOpt.auto=true;
		arr.push({ja: ja, jb: jb, width: width, cellClass:colOpt.cellClass});
//		console.log(colOpt);
	});
	dc.body2.hide(); // 隐藏表主体部分以减少渲染开销(ss.set将修改style，会导致立即渲染, 表的行多列多时非常慢)
	$.each(arr, function (i, e) {
		e.ja.width(e.width);
		dgData.ss.set("."+e.cellClass, (e.width+20)+"px"); // NOTE: hardcode 20
//		e.jb.width(e.width);
	});
	dc.body2.show();
//	jtbl.datagrid("fitColumns");
}
...
}else if (opts.quickAutoSize) {
quickAutoSize(_7b6, _7b8);
}
else {
var _7b9=false;
...
// >>>> lj:perfix: fixGridLoadPerf
$(_7b6).datagrid("fixRowHeight");
}
tmp.remove();
function _70f(_7bb){
...
_7c6(_7c2);
// >>>> lj:perfix: fixGridLoadPerf: grid loading is slow when it has many columns
if (!_7c3) _725(_7c2); // skip fixRowHeight (too slow) if for each column(_7c3 is column name)
_7c7(_7c2);
function fix(_7c8){
...
// >>>LJ: opt.sumRow=1 means the row wont be sort
if (opts.sumRow && data.rows.length > 1) {
	var sumRow = data.rows.pop();
}
...
// >>>LJ: null is better to be special treated
if (r1[sn] === r2[sn])
	r = 0;
else if (r1[sn] === null)
	r = -1;
...
// >>>LJ
if (opts.sumRow && sumRow) {
	data.rows.push(sumRow);
}
}
...
// >>>LJ extension
opts.onInitOptions.call(this, opts);
_743(this);
_75b(this);
_70f(this);
if(opts.data){
...
var jo = $(_8df).empty().html(this.renderTable(_8de,0,rows,_8e0));
// >>>LJ: set datagrid max width
jo.addClass("wui-datagrid-cell-max-width");
setTimeout(function () {
	jo.removeClass("wui-datagrid-cell-max-width");
});
// <<<
...
$(_9c6).datagrid($.extend({},opts,{url:null,data:null,
//>>>LJ: treegrid支持在onInitOptions中添加UDF
onInitOptions: $.fn.datagrid.defaults.onInitOptions,
url1:opts.url, // instead of url for treegrid
//<<<
loader:function(){
return false;
