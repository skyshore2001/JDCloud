...
列车查询接口示例如下:
	...
	Hub.query() -> tbl(id, ..., lastTm?, lastPos?, @hubData?, %lastData?)
...
- lastData: {id,tm,pos} 最近一次接收的数据。可用`param_lastData`指定查询条件。适用于大量数据查询（如全表查询不分页时）。

- lastTm, lastPos: 最近一次数据。适用于少量数据查询（如get接口或分页的query接口），对于大量数据查询（如导出全表）性能差。
- lastTm2, lastPos2: 最近一次数据。适用于大量数据查询，实现原理与lastData相同。
...
各种实现参考：
	...
	SELECT hubId, pos, tm FROM (SELECT * FROM HubData, (select @n:=0) t_ ORDER BY hubId,id DESC) HubData1 GROUP BY hubId
		...
		protected $subobj = [
			...
			"lastData" => ["obj"=>"HubData", "cond"=>"hubId=%d", "AC"=>"AC2_LastHubData", "res"=>"id,tm,pos", "orderby"=>"t0.id DESC", "wantOne"=>true ],

			// 仅供参考，不建议作为产品实现。lastData2只是hubData加上了wantOne属性（也可由前端直接使用hubData并指定wantOne），其限制同hubData
			"lastData2" => ["obj"=>"HubData", "cond"=>"hubId=%d", "AC"=>"AC2_HubData", "res"=>"id,tm,pos", "orderby"=>"t0.id DESC", "wantOne"=>true ]
		];
	}
...
为了方便，常常还在列表页中添加导入按钮，只导入指定对象，且导入后刷新当前列表，代码如下：pageEmployee.js:
