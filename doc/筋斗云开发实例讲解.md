...
首先，在列表页初始化函数initPageXXX中添加参数，如（pageItem.js）：
	...
	function initPageItem(objParam)
	{
		...
		// 控制列显示
		toggleCol(jtbl, 'type', !type);
		toggleCol(jtbl, 'status', !type || type!="公告");
		...
		// 建议写 toggleItemFields(jtbl, type);
	}
	...
	function initDlgItem()
	{
		...
		function onBeforeShow(ev, formMode, opt) 
		{
			...
			function onShow() {
				...
				...
				// 建议写 toggleItemFields(jfrm, type);
...
建议使用toggleFields工具函数，它可同时适用于列表页或明细页上的字段显示控制。
