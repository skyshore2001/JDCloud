...
修改明细对话框。在dlgItem.html中，为type字段设置类wui-fixedField，这样该字段会自动根据opt.objParam来设置值:

	<input name="type" class="wui-fixedField" ...>
...
参照例子，应特别注意在列表页和明细页上toggleItemFields这段代码的位置。

### 对话框上根据类型显示不同的下拉列表

参考mycombobox和wui-combogrid的setOption事件：动态修改下拉框的选项，实现动态更新查询等。
...
在pageOrdr.html中增加弹出菜单：
	...
	<div class="menu" style="width:150px;display:none">
			<div data-options="iconCls:'icon-save'">重新生成配置</div>
...
在pageOrdr.js中初始化菜单并添加处理函数：
	...
	var jmenu = jpage.find(".menu").menu({
		onClick: function (o) {
			// console.log(o);
			var name = o.text;
			switch (name) {
	...
	var btnMore = {text: "更多", iconCls:'icon-more', class="menubutton", menu: jmenu};
...
有如下订单表和队列表：
	...
	queue
	: List(orderId). storeId店当前排队队列中的订单，如"100,101"
...
设计：增加虚拟字段：
	...
	@Queue: id, ...
	vcol(ForSearch): storeQueueId?, queueOrder/队列顺序(for orderby)

这样，查询就可以用

	callSvr("Ordr.query", {
		cond: "storeQueueId=" + id,
		orderby: 'queueOrder',
	});
...
实现：
	...
	class AC2_Ordr extends AccessControl
	{
		...
		protected $vcolDefs = [
			[
				"res" => ["q.storeId storeQueueId", "find_in_set(t0.orderNo, q.queue) queueOrder"],
				"join" => "JOIN Queue q ON find_in_set(t0.orderNo, q.queue)"
			]
		];
	}

这里用了find_in_set作为过滤条件。同时它刚好返回在字符串中出现的位置，刚好可以用于排序条件。
