...
- uniKey: 唯一索引字段. 如果指定, 则以该字段查询记录是否存在(调用query接口）, 存在则更新（调用set接口）。例如"code", 也支持多个字段（用于关联表），如"bpId,itemId"。
 ...
 特别地，如果uniKey以"!"结尾（如"code!"），则变成**更新模式**，表示必须匹配到记录，不匹配则报错而不做添加，常用于通过导入批量更新数据。
 不加!后缀时虽然也可更新记录，但若不匹配时会误添加记录。
...
- id: 被复制对象的id或id列表（以逗号分隔），示例: `100`, `100,101`
