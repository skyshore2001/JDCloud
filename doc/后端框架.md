...
下面用Obj代指对象实际名称。
	...
	查询列表(默认压缩表格式)：
	Obj.query(res?, cond?, distinct?=0, _pagesz?=20, _pagekey?, _fmt?) -> tbl(fields...) = {nextkey?, total?, @h, @d}

	查询列表 - 对象列表格式：
	Obj.query(_fmt=list, ...) -> {nextkey?, total?, @list=[obj1, obj2...]}
	...
	导出文件：
	Obj.query(_fmt=csv/txt/excel, ...) -> 文件内容
